...
export function useDashboardData() {
  ...
  return useAsyncData<DashboardData>('dashboard-data', async () => {
    ...
    const [transactionsResult, customersResult] = await Promise.all([
      supabase
        .from('transactions')
        .select('id, nomor_bon, tanggal, ongkir, status, is_bonus, customer_id, transaction_lines(line_omzet)')
    ...
    const customers = (customersResult.data ?? []) as unknown as CustomerRecord[]
    const customerNames = new Map(customers.map((customer) => [customer.id, customer.nama]))
    ...
    const lunasThisMonth = transactions.filter(
      (transaction) =>
        transaction.status === 'lunas' && !transaction.is_bonus && isCurrentMonth(transaction.tanggal),
    )
    const totalPiutang = piutangTransactions.reduce(
      (total, transaction) => total + lineTotal(transaction) + numberFrom(transaction.ongkir),
      0,
    )
    ...
    const customersWithPiutang = new Set(piutangTransactions.map((transaction) => transaction.customer_id))

    const paidOmzetByCustomer = transactions
      .filter((transaction) => transaction.status === 'lunas' && !transaction.is_bonus)
      .reduce((map, transaction) => {
        map.set(transaction.customer_id, (map.get(transaction.customer_id) ?? 0) + lineTotal(transaction))
        return map
    ...
    return {
      ...
      metrics: [
        ...
        {
          label: 'Pelanggan Piutang',
          value: String(customersWithPiutang.size),
          helper: 'Perlu ditagih atau dicek',
          icon: 'lucide:users',
        },
        {
          label: 'Bonus Tersedia',
      ...
      urgentReceivables: piutangTransactions.slice(0, 5).map((transaction) => ({
        id: transaction.id,
        title: transaction.nomor_bon,
        meta: `${customerNames.get(transaction.customer_id) ?? 'Pelanggan'} - ${formatDate(transaction.tanggal)}`,
        amount: formatRp(lineTotal(transaction) + numberFrom(transaction.ongkir)),
        status: 'piutang',
      })),
      bonusCustomers: bonusCustomers.slice(0, 5).map(({ customer, available }) => ({
        id: customer.id,
        title: customer.nama,
