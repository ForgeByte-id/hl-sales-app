<template>
  <AppShell>
    <div class="space-y-5">
      <PageHeader
        title="Pelanggan"
        description="Kelola pelanggan, diskon, piutang, dan bonus."
        eyebrow="Master Data"
      >
        <template #actions>
          <AppButton to="/customers/new" icon="lucide:user-plus"
            >Tambah Pelanggan</AppButton
          >
        </template>
      </PageHeader>
      ...
      <SectionPanel
        ...
        description="Pelanggan aktif untuk pembuatan Bon."
      >
          ...
          >
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
              <tr v-for="customer in filteredCustomers" :key="customer.id">
                ...
                <td class="px-3 py-3 text-neutral-600 dark:text-neutral-300">
                  {{ formatRp(receivableByCustomer.get(customer.id) ?? 0) }}
                </td>
                <td class="px-3 py-3">
                  <AppBadge
                    v-if="bonusByCustomer.get(customer.id)"
                    tone="bonus"
                  >
...
async function archiveCustomer(id: string) {
  const confirmed = await confirm({
    ...
    description:
      "Riwayat transaksi tetap tersimpan dan pelanggan tidak muncul di pilihan Bon baru.",
  ...
  const { error } = await supabase
    ...
    .eq("id", id);
  if (error) {
    toast.error("Pelanggan belum bisa diarsipkan.");
    return;
  }

  toast.success("Pelanggan diarsipkan.");
  await refresh();
}
</script>
