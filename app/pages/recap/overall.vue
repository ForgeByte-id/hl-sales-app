<template>
  <AppShell>
    <div class="space-y-5">
      <PageHeader title="Rekap Keseluruhan" description="Ringkasan seluruh pelanggan." eyebrow="Laporan">
        <template #actions>
...
const { data: transactions } = await useAsyncData('recap-overall', async () => {
...
const summary = computed(() => {
  const paid = filtered.value.filter((transaction) => transaction.status === 'lunas' && !transaction.is_bonus)
  const receivable = filtered.value.filter((transaction) => transaction.status === 'piutang' && !transaction.is_bonus)
