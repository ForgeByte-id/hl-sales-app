<template>
  <AppShell>
    <div class="space-y-5">
      <PageHeader :title="`Rekap ${customer?.nama || 'Pelanggan'}`" description="Ringkasan per pelanggan." eyebrow="Laporan">
        <template #actions>
...
const { data } = await useAsyncData(`recap-customer-${customerId}`, async () => {
  const [customerResult, transactionResult] = await Promise.all([
    supabase.from('customers').select('*').eq('id', customerId).single(),
    supabase.from('transactions').select('*, transaction_lines(*)').eq('customer_id', customerId).order('tanggal', { ascending: false }),
  ])
...
function omzet(transaction: { transaction_lines?: Array<{ line_omzet: number | string }> }) {
  return (transaction.transaction_lines ?? []).reduce((total, line) => total + toNumber(line.line_omzet), 0)
}

function profit(transaction: { transaction_lines?: Array<{ line_laba_hl: number | string }> }) {
  return (transaction.transaction_lines ?? []).reduce((total, line) => total + toNumber(line.line_laba_hl), 0)
}

const summary = computed(() => {
  const paid = filtered.value.filter((transaction) => transaction.status === 'lunas')
  const receivable = filtered.value.filter((transaction) => transaction.status === 'piutang')
  ...
  return {
    ...
    piutang: receivable.reduce((total, transaction) => total + omzet(transaction) + toNumber(transaction.ongkir), 0),
    paid: paid.reduce((total, transaction) => total + omzet(transaction) + toNumber(transaction.ongkir), 0),
  }
})

const summaryCards = computed(() => [
  { label: 'Omzet Lunas', value: formatRp(summary.value.omzet), helper: 'Ongkir tidak dihitung', icon: 'lucide:chart-column' },
  ...
  { label: 'Sudah Dibayar', value: formatRp(summary.value.paid), helper: 'Omzet + ongkir', icon: 'lucide:badge-check' },
])
