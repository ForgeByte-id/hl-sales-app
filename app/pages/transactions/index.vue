<template>
  <AppShell>
    <div class="space-y-5">
      <PageHeader
        title="Transaksi"
        description="Daftar Bon dan status pembayaran."
        eyebrow="Bon"
      >
        <template #actions>
      ...
      <SectionPanel title="Daftar Bon">
        <div class="mb-4 grid gap-3 md:grid-cols-[1fr_180px]">
          <AppTextInput
            v-model="search"
            label="Cari"
            placeholder="Nomor Bon atau pelanggan"
            icon="lucide:search"
          />
          <label class="block">
            <span
...
const filteredTransactions = computed(() => {
  ...
  return (transactions.value ?? []).filter((transaction) => {
    const haystack =
      `${transaction.nomor_bon} ${transaction.customers?.nama || ""}`.toLowerCase();
...
async function deleteTransaction(id: string) {
  const confirmed = await confirm({
    title: "Hapus Bon?",
    description: "Data Bon dan baris produknya akan dihapus dari sistem.",
    confirmLabel: "Hapus Bon",
    tone: "danger",
  });
  if (!confirmed) return;
