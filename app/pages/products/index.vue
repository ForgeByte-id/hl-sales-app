<template>
  <AppShell>
    <div class="space-y-5">
      <PageHeader
        title="Produk"
        description="Kelola produk aktif untuk Bon."
        eyebrow="Master Data"
      >
        <template #actions>
          <AppButton to="/products/new" icon="lucide:package-plus"
      ...
      <SectionPanel title="Daftar Produk">
        ...
        <EmptyState
          ...
          description="Tambahkan produk agar bisa dipilih pada Bon."
          icon="lucide:package"
        />
          ...
          >
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-neutral-800">
              <tr v-for="product in filteredProducts" :key="product.id">
                ...
                <td class="px-3 py-3">
                  <AppBadge>{{ product.tipe }}</AppBadge>
                </td>
                <td class="px-3 py-3 text-neutral-600 dark:text-neutral-300">
                  {{ formatRp(toNumber(product.harga_base)) }}
                </td>
                <td class="px-3 py-3">
...
const filteredProducts = computed(() => {
  ...
  return (products.value ?? []).filter((product) => {
    const matchesKeyword =
      !keyword || product.nama.toLowerCase().includes(keyword);
...
async function archiveProduct(id: string) {
  const confirmed = await confirm({
    ...
    description:
      "Riwayat transaksi tetap tersimpan dan produk tidak muncul di pilihan Bon baru.",
  ...
  const { error } = await supabase
    ...
    .eq("id", id);
  if (error) {
    toast.error("Produk belum bisa diarsipkan.");
    return;
  }
